...
@section('content')
    ...
    font-weight: bold;">@lang('main.Follow-up record of senior management decisions')</h3>
                <hr>
            </div>
            <div class="row" style='margin:auto;width:90%'>

                <a href="{{ route('followLog.create') }}" class="btn col-md-12 mr-1"
                    style=" ">
                                            ...
                                            <table id="example1" class="table  table-striped shadow-lg">
                                                ...
                                                <tbody class="datatable-body ">
                                                    <tbody class="datatable-body text-center ">
                                                        @foreach ($all_followLog as $followLog)
                          <tr class="datatable-row datatable-row-even">
                              <td class="datatable-cell" style="font-size:15px "><span><img src="{{asset($followLog->logo)}}" alt="Image" width="50px"></span></td>
                              <td class="datatable-cell" style="font-size:15px ">
                                <span>{{ $followLog->status }}</span>
                            </td>
                            @if (Auth::user()->hasRole('Employee'))
                                                    <td>
                                                        <form id="delete-form-{{ $followLog->id }}"
                                                            ...
                                                            <a href="{{ route('followLog.print', $followLog->id) }}"
                                                            ...
                                                            <button type="button" class="btn btn-sm btn-clean btn-icon"
                                                                ...
                                                                onclick="confirmDelete('delete-form-{{ $followLog->id }}')">
                                            ...
                                            btn-icon mr-2"
                                                            ...
                                                            @csrf
                                                            @method('delete')
                                                            @if($followLog->status == 'inProgress' && Auth::user()->hasRole('Admin') || $followLog->status == 'pending' && Auth::user()->hasRole('Admin'))
                                                            <button type="button" class="btn btn-sm btn-clean btn-icon"
                                                                title="@lang('general.delete')"
                                                                onclick="confirmDelete('delete-form-{{ $followLog->id }}')">
                                                                <i class="fa fa-trash"></i>
                                                     ...
                                                     @elseif($followLog->status == 'inProgress' && Auth::user()->hasRole('SuperAdmin') || $followLog->status == 'pending' && Auth::user()->hasRole('SuperAdmin')
